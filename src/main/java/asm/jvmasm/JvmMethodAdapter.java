...
import jvm.GotoState;
import jvm.LocalVariables;
import jvm.OperandStack;
...
public class JvmMethodAdapter<T> extends MethodVisitor {
    ...
    public JvmMethodAdapter(InheritanceMap inheritanceMap,
                            Map<MethodReference.Handle, Set<Integer>> passthroughDataflow,
                            final int api, final MethodVisitor mv, final String owner,
            ...
            case Opcodes.INVOKEINTERFACE:
                ...
                for (int i = 0; i < argTypes.length; i++) {
                    ...
                    if (argType.getSize() > 0) {
                        ...
                        argTaint.set(argTypes.length - 1 - i, operandStack.pop());//把操作数栈的数据pop出来存到argTaint，因为这个数据必然是从本地变量表中取出来的
                    }
                }
                ...
                if (retSize > 0) {
                    operandStack.push(resultTaint);//污染结果入栈
                    for (int i = 1; i < retSize; i++) {
                        operandStack.push();//污点分析的逻辑就是额外push一份数据
                    }
                }
                break;
            default:
